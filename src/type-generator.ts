 ...
 */
export async function generateTypesFromAPI(
  options: I18nClientModuleOptions,
  outputPath?: string
): Promise<void> {
...
`;
      ...
      if (fs.existsSync(finalOutputPath)) {
        ...
        const existingContentNormalized = existingContent.replace(
          /Last generated: [^\n]*/g,
          ''
        );
        const newContentNormalized = fallbackContent.replace(
          /Last generated: [^\n]*/g,
          ''
        );
...
`;
    ...
    if (fs.existsSync(finalOutputPath)) {
      ...
      const newContentNormalized = fileContent.replace(
        /Last generated: [^\n]*/g,
        ''
      );
    ...
    console.warn(
      `[nestjs-i18n-client] Failed to generate types: ${error.message}`
    );
  }
}
